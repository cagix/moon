...
(defn -main []
  ...
  (cdq.create.extend-protocols/do! Context 'cdq.ctx)
  (lwjgl/start-application! {:title "Cyber Dungeon Quest"
                             :windowed-mode {:width 1440 :height 900}
                             :foreground-fps 60}
                            {:create! (fn [context]
                                        (reset! cdq.application/state (create! context)))
                             :dispose! (fn []
                                         (dispose! @cdq.application/state))
                             :render! (fn []
                                        (swap! cdq.application/state render!))
                             :resize! (fn [width height]
                                        (resize! @cdq.application/state width height))}))
