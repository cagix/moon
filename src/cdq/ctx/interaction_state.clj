...
(defn skill-interaction-state [ctx eid skill-id]
  (let [entity @eid
  ...
  (let [[k params] (interaction-state ctx player-eid)]
    (case k
      ...
      :interaction-state/clickable-mouseover-eid (let [mouseover-eid params
                                                   ...
                                                   cursor)
      :interaction-state.skill/usable :cursors/use-skill
      :interaction-state.skill/not-usable :cursors/skill-not-usable
      :interaction-state/no-skill-selected :cursors/no-skill-selected)))
  ...
  (let [[k params] (interaction-state ctx player-eid)]
    (case k
      ...
      :interaction-state/clickable-mouseover-eid (let [mouseover-eid params
                                                   ...
                                                   txs)
      :interaction-state.skill/usable (let [[skill effect-ctx] params]
                                        [[:tx/event player-eid :start-action [skill effect-ctx]]])
      :interaction-state.skill/not-usable (let [state params]
                                            [[:tx/sound "bfxr_denied"]
                                             [:tx/show-message (case state
                                                                 :cooldown "Skill is still on cooldown"
                                                                 :not-enough-mana "Not enough mana"
                                                                 :invalid-params "Cannot use this here")]])
      :interaction-state/no-skill-selected [[:tx/sound "bfxr_denied"]
