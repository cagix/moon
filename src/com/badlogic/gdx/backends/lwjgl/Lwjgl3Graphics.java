...
public class Lwjgl3Graphics extends AbstractGraphics implements Disposable {
	...
	public volatile int backBufferWidth;
	public volatile int backBufferHeight;
	public volatile int logicalWidth;
	public volatile int logicalHeight;
	...
	public IntBuffer tmpBuffer = BufferUtils.createIntBuffer(1);
	public IntBuffer tmpBuffer2 = BufferUtils.createIntBuffer(1);

	public GLFWFramebufferSizeCallback resizeCallback = new GLFWFramebufferSizeCallback() {
		@Override
		public void invoke (long windowHandle, final int width, final int height) {
			if (!"glfw_async".equals(Configuration.GLFW_LIBRARY_NAME.get())) {
				Clojure.var("com.badlogic.gdx.backends.lwjgl", "update-framebuffer-info").invoke(this);
				if (!window.isListenerInitialized()) {
					return;
				}
				Clojure.var("com.badlogic.gdx.backends.lwjgl", "make-current!").invoke(window);
	...
	@Override
	public int getWidth () {
		if (window.hdpiMode == HdpiMode.Pixels) {
	...
	public static class Lwjgl3DisplayMode extends DisplayMode {
		...
		public Lwjgl3DisplayMode (long monitor, int width, int height, int refreshRate, int bitsPerPixel) {
