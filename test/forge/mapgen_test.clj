...
#_(defn- map-infos ^String [c]
  (let [tile (mapv int (c/world-mouse-position ctx))
        {:keys [tiled-map
...
#_(defn- render-on-map [_context]
  (let [{:keys [tiled-map
        ...
        visible-tiles (c/visible-tiles ctx)
        [x y] (mapv int (c/world-mouse-position ctx))]
...
#_(defn ->generate-map-window [c level-id]
  (ui/window {:title "Properties"
              ...
              :rows [[(ui/label (with-out-str (pprint (db/build db level-id ctx))))]
                     [(text-button "Generate" #(try (generate-screen-ctx c (db/build db level-id ctx))
                                                    (catch Throwable t
                                                      (pretty-pst t)
