...
(deftest modify-source-damage
  (is (= (->value {:entity/modifiers {:modifier/damage-deal-min {:op/inc 1
                                                                 :op/mult 50}}}
                  {:damage/min-max [5 10]})
         #:damage{:min-max [9 10]})))

(deftest modify-source-damage-align
  (is (= (->value {:entity/modifiers {:modifier/damage-deal-min {:op/inc 20}}}
                  {:damage/min-max [5 10]})
         #:damage{:min-max [25 25]})))

(deftest modify-source-damage-align-negative
  (is (= (->value {:entity/modifiers {:modifier/damage-deal-min {:op/mult -100}
